...
export default class Strategies {
    ...
    private static LEVEL_QUALITY_ON: boolean = true;   // Flag to indicate if we multiply the score by the level's quality
    public static LOD_ON: boolean = false;               // If true, we will only import the maximum level of the objects

    public static executionTime: any = {};
    ...
    public static async executeStrategy(object_manager: ObjectManager): Promise<Mesh[]> {
        let currentStrat = this.getChosenStrategyAsString();
        let currentMetr = Metrics.getChosenMetricAsString();

        if (!this.executionTime[currentStrat]) {
            this.executionTime[currentStrat] = {};
        }
        if (!this.executionTime[currentStrat][currentMetr]) [
            this.executionTime[currentStrat][currentMetr] = {
                calculation: 0,
                calcAndImport: 0,
                nb: 0
            }
    ...
    private static async naive1(object_manager: ObjectManager): Promise<Mesh[]> {
        ...
        for (let i = 0; i < visibles.length; i++) {
            ...
            for (let j = (Strategies.LOD_ON) ? 
                    currentObject.currentLevel : 
                    currentObject.getNumberOfLevels() - 1; 
                j < currentObject.getNumberOfLevels(); 
                j++) {

                if (!currentObject.getLevel(j)) {
                    // Score = utility right now * Level's quality score
                    const utility = (this.LEVEL_QUALITY_ON ? currentObject.getLevelQuality(j) : 1) *
        ...
        this.executionTime[currentStrat][currentMetr].calculation += (performance.now() - startTime);
        ...
        this.executionTime[currentStrat][currentMetr].calcAndImport += (performance.now() - startTime);
        this.executionTime[currentStrat][currentMetr].nb += 1;
            ...
            for (let j = (Strategies.LOD_ON) ? 
                ...
                if (!currentObject.getLevel(j)) {
                    ...
                    if (increment > 0) {
                        ...
                        utility *= (this.LEVEL_QUALITY_ON ? currentObject.getLevelQuality(j) : 1);
                        ...
                        const utility = Metrics.calcUtility(currentObject, nextCam) 
                                        * (this.LEVEL_QUALITY_ON ? currentObject.getLevelQuality(j) : 1)            // The level's quality score
                                        / t_next;
                        nextCam.dispose();
    ...
    private static async greedy2(object_manager: ObjectManager): Promise<Mesh[]> {
        ...
        for (const obj of objects) {
            utilities.push({ 
                object: obj, 
                utility: Metrics.calcUtility(obj, cam), 
                nextLevel: (Strategies.LOD_ON) ? 
                    obj.currentLevel : 
                    Math.max(obj.currentLevel, obj.getNumberOfLevels() - 2) // Minus 2, so that, in the loop, we test NumberOfLevels - 1 
            });
        };
