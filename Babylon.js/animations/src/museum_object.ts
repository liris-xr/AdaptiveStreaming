...
import { DracoCompression, TransformNode, Mesh, Geometry, VertexData, VertexBuffer } from "@babylonjs/core/Meshes/";
...
export default class MuseumObject {
    ...
    public reset(): void {
        if (this.levels && this.asked && this.metadata) {
            for (let lev = 1; lev < this.levels.length; lev ++) {
                if (this.levels[lev]) {
                    this.levels[lev].dispose();
                }
            }

            (Strategies.LOD_ON) ?
                this.levels[0].setEnabled(true):
                this.levels[0].setEnabled(false);
            
            let levelsT = new Array(this.metadata.nb_levels);
            let askedT = new Array(this.metadata.nb_levels);

            levelsT[0] = this.levels[0];
            askedT[0] = this.asked[0];
    ...
    private surfaceArea(mesh: Mesh): number {
        if (!mesh) {
            return 0.0;
        }
        var ar = 0.0;
        ...
        for (var i = 0; i < nbFaces; i++) {
            ar = ar + this.facetArea(mesh, i, nbFaces);
        }
        return ar;
    };
    
    private facetArea(mesh: Mesh, faceId: number, nbFaces: number): number {
        if(!mesh) {
            return 0.0;
        }
        var indices = mesh.getIndices()!;
