...
export default class DracoImporter {
    ...
    private meshes: Array<BABYLON.Mesh> | null;
    ...
    async _init_(): Promise<void> {
        if (this.folderPath != null) {
            ...
            if (pMetadata) {
                ...
                throw "Couldn't find metadata";
            }

            this.meshes = new Array(this.metadata.nb_levels);
    ...
    async import(level: number): Promise<BABYLON.Mesh | null> {
        if (level < 0 || level > this.metadata.nb_levels || (this.meshes && this.meshes[level])) return null;
        ...
        if (this.meshes) {
            ...
            if (level > this.activeLevel) {
                if (this.activeLevel != -1) this.meshes[this.activeLevel].setEnabled(false);
                this.activeLevel = level;
            }
            else {
                this.meshes[level].setEnabled(false);
            }
        }
