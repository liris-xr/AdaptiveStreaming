...
export default class Strategies {
    ...
    private static async naive1(object_manager: ObjectManager): Promise<Mesh[]> {
        ...
        for (let i = 0; i < visibles.length; i++) {
            ...
            for (let j = 0; j < currentObject.getNumberOfLevels(); j++) {
    ...
    private static async greedy2(object_manager: ObjectManager): Promise<Mesh[]> {
        ...
        for (let i = 0; i < utilities.length; i++) {
            ...
            console.log(currentObject.getCurrentMesh().name);
        ...
        console.log((performance.now() - zero) / 1000);
    ...
    private static async uniform2(object_manager: ObjectManager): Promise<Mesh[]> {
        ...
        for (const obj of objects) {
            utilities.push({ object: obj, utility: Metrics.calcUtility(obj, cam), nextLevel: 0 });
        };
    ...
    private static async hybrid2(object_manager: ObjectManager): Promise<Mesh[]> {
        ...
        for (const obj of visibles) {
            visibleUtilities.push({ object: obj, utility: Metrics.calcUtility(obj, cam), nextLevel: 0 });
        };
        for (const obj of notVisibles) {
            notVisibleUtilities.push({ object: obj, utility: Metrics.calcUtility(obj, cam), nextLevel: 0 });
        };
